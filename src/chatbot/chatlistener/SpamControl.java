...
{
    private ArrayList<String> _bannablePhrases = new ArrayList<>();
    private HashMap<String,Date> _permittedUsers = new HashMap<>();
    ...
    {
        _bannablePhrases = populatePhrases();
        _filterActive = false;
        register(sub);
    }
    ...
    public void onMessage(MessageEvent message) 
    {
        ...
        if (!message.getChannel().getOps().contains(message.getUser()) && _filterActive) 
        {
            for (String test : _bannablePhrases)
            {
                if (currentMessage.contains(test)) 
                {
                    System.out.println(message.getUser().getNick());
                    if (!_permittedUsers.containsKey(message.getUser().getNick()) || _permittedUsers.get(message.getUser().getNick()).before(new Date(System.currentTimeMillis()))) 
                    {
                        purge(message);
                        break;
                    }
    ...
    private String permitUser(String user) 
    {
        ...
        _permittedUsers.put(user, endTime);
